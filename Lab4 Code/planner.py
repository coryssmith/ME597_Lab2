...
class planner:
    ...
    def trajectory_planner(self, startPoseCart, endPoseCart):
        ...
        # Wrap the Cartesian pose in an array so that position_2_cell returns an array,
        # then extract the first (and only) element.
        ...
        path = search(self.costMap, startPose, endPose) #self.m_utilites.getMap()
        if path is None:
            return None
