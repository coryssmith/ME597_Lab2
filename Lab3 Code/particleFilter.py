...
class particleFilter(Node):
    ...
    def __init__(self, mapFilename="your_map/room.yaml", numParticles=500):
        ...
        self.mapUtilities = mapManipulator(mapFilename, laser_sig= 5)
    ...
    def initializeParticleFilter(self, x, y, th):
        ...
        # TODO: Generate the particle noise around the initial pose (x, y, th) (you should use the standard deviation from particles
        # std_particle_x, std_particle_y, std_particle_theta) - Gaussian noise
        self.particlePoses = np.zeros((numParticles, 3)) #size should be (numParticles, 3)
        
        self.particlePoses[:,0] =  np.random.normal(x, self.std_particle_x, numParticles)
        self.particlePoses[:,1] =  np.random.normal(y, self.std_particle_y, numParticles)
        self.particlePoses[:,2] =  np.random.normal(th, self.std_particle_theta, numParticles)
    ...
    def resample(self, laser_scan, mapUtilInstance):
        ...
        # Enabling replacement to allow particles with high weight to replicate
        sampled_particles = np.random.choice(self.particles, size = self.numParticles, replace = True, p=particles_weights)

        for bp in sampled_particles:
            x, y, th = bp.getPose()
            # TODO: add noise to the x, y, and th, use the same std_noise for x, y, and th
            # Don't concentrate all the particles to the depth of the particle (?)
